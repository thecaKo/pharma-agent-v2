...
const SPECS: ToolSpec[] = [
  ...
  { name: "fs.grep", command: "fs.grep", description: "Busca regex no conteúdo de arquivos sob um diretório (pula binário; deny-list de roots).", inputSchema: { type: "object", required: ["root", "pattern"], properties: { root: { type: "string" }, pattern: { type: "string" }, ignoreCase: { type: "boolean" }, extensions: ARRAY, maxDepth: { type: "integer" }, maxFileBytes: { type: "integer" }, maxMatches: { type: "integer" }, maxFilesScanned: { type: "integer" } } }, outputSchema: { type: "object", properties: { matches: ARRAY, truncated: { type: "boolean" }, filesScanned: { type: "integer" }, errors: ARRAY } } },
  { name: "fs.find", command: "fs.find", description: "Localiza arquivos por glob de nome em múltiplas raízes (deny-list + skip dirs).", inputSchema: { type: "object", required: ["roots", "namePatterns"], properties: { roots: ARRAY, namePatterns: ARRAY, maxDepth: { type: "integer" }, maxResults: { type: "integer" } } }, outputSchema: { type: "object", properties: { files: ARRAY, truncated: { type: "boolean" }, rootsRejected: ARRAY, errors: ARRAY } } },
  { name: "schema.search", command: "schema.search", description: "Localiza tabelas/colunas por keyword via information_schema (LIKE %keyword%). Não executa SQL arbitrário.", inputSchema: { type: "object", required: ["keywords"], properties: { keywords: ARRAY, schema: { type: "string" }, maxTables: { type: "integer" } } }, outputSchema: { type: "object", properties: { tables: ARRAY } } },
  { name: "schema.listSchemas", command: "schema.listSchemas", description: "Lista os schemas do banco via information_schema.schemata.", inputSchema: OBJECT, outputSchema: { type: "object", properties: { schemas: ARRAY } } },
  { name: "util.decode", command: "util.decode", description: "Decodifica uma string em hex, base64 ou URL-encode (rejeita resultado binário).", inputSchema: { type: "object", required: ["value", "encoding"], properties: { value: { type: "string" }, encoding: { type: "string", enum: ["hex", "base64", "url"] } } }, outputSchema: { type: "object", properties: { ok: { type: "boolean" }, encoding: { type: "string" }, decoded: { type: "string" }, errorCode: { type: "string" } } } }
];
